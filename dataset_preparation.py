...
def mol_to_graph_data_obj_simple_3D(mol, type_view):
    ...
    if type_view in ['3D', 'fingerprint']:
    ...
    def process(self):
        ...
        for smiles, sub_dic in tqdm(drugs_summary):
            ...
            with open(mol_path, 'rb') as f:
                ...
                if self.type_view == '3D':  # 3D datasets
                ...
                elif self.type_view == 'fingerprint': 
                    for conformer_dict in conformer_list[:self.n_conf]:
                        # select the first n_conf conformations
                        rdkit_mol = conformer_dict['rd_mol']
                        data = mol_to_graph_data_obj_simple_3D(rdkit_mol, self.type_view)
        ...
        # # === fingerprints and smiles tokenizer ===
        # elif self.type_view in ['fingerprint', 'smiles']: 
        #     for smiles, _ in tqdm(drugs_summary):  
        #         rdkit_mol = Chem.MolFromSmiles(smiles)             
        #         data = mol_to_graph_data_obj_simple_3D(rdkit_mol, self.type_view)
        #         if self.type_view == 'fingerprint':
        #             # === Get molecules' fingerprint and 2D structure ===          
        #             fingerprint = np.array(GetMorganFingerprintAsBitVect(rdkit_mol, self.radius, nBits=self.nBits))  
        #             data.fingerprint = torch.tensor(fingerprint, dtype = torch.float).unsqueeze(0)
        #         data.id = torch.tensor([idx])
        #         data.mol_id = torch.tensor([mol_idx])
        ...
        if self.type_view == 'smiles':
        # === SMILES tokenize and padding ===
        # TODO: More efficient processing needed 
            # tokenizer = RobertaTokenizerFast.from_pretrained("roberta-base")
            tokenizer = AutoTokenizer.from_pretrained("seyonec/ChemBERTa-zinc-base-v1")
            inputs = tokenizer(data_smiles_list, return_tensors="pt", padding='longest')
            # inputs format : {input_ids: [], attention_mask:[]}
            for i in range(len(data_list)):
                data_list[i].input_ids = inputs['input_ids'][i].unsqueeze(0)
                data_list[i].attention_mask = inputs['attention_mask'][i].unsqueeze(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type_view', type=str, default='fingerprint', help='type of processed dataset',
                        choices=['3D', 'fingerprint', 'smiles'])
    ...
    if args.sum:
        ...
        # n_mol, n_conf = 1000000, 5
            root = '../datasets/GEOM/processed/GEOM_3D_nmol%d_nconf%d_nupper%d/' % (n_mol, n_conf, n_upper)
        elif args.type_view == 'fingerprint':
            root = '../datasets/GEOM/processed/GEOM_fp&3D_nmol%d_nconf%d_nupper%d/' % (n_mol, n_conf, n_upper)
        ...
        MoleculeDataset(root=root, n_mol=n_mol, n_conf=n_conf, n_upper=n_upper, type_view = args.type_view)
