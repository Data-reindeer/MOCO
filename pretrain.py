...
from models.model2D import GNN, MLP, Multi_View_Fusion, FingerPrintEncoder
...
def train(args, molecule_model_2D, model_contrast, type_view, device, loader, optimizer):
    ...
    for step, batch in enumerate(l):
        ...
        if type_view == '3d':
            ...
            molecule_repr = molecule_model_contrast(batch.fingerprint)
            CL_loss, CL_acc = dual_CL(molecule_2D_repr, molecule_repr, args)
...
if __name__ == '__main__':
    ...
    # dir_name = '[lr{}]2d{}_3d{}_fp{}_sm{}_fuse{}_params/'.format(args.lr, args.gnn_lr_scale, args.schnet_lr_scale, 
    #                                                         args.fp_lr_scale, args.mlp_lr_scale, args.fuse_lr_scale)
    dir_name = 'Classification_models/'
    ...
    print('Avg atoms:{}'.format(dataset.data.x.shape[0]/dataset.data.id.shape[0]))
    print('Avg bond:{}'.format((dataset.data.edge_index.shape[1]/2)/dataset.data.id.shape[0]))
    if dataset.data.x.shape[0] > 0:
        raise Exception('Stop statistic')
    ...
    # === Multi-view Fusion via Attention ===
        ...
        model_param_group.append({'params': molecule_model_contrast_fp.parameters(), 'lr': args.lr * args.fp_lr_scale}) 
        model_param_group.append({'params': molecule_model_contrast_3d.parameters(), 'lr': args.lr * args.schnet_lr_scale}) 
