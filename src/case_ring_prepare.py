    ...
    def process(self):
        ...
        for smiles, sub_dic in tqdm(drugs_summary):
            ...
            with open(mol_path, 'rb') as f:
                ...
                conformer_dict = conformer_list[0]
                rdkit_mol = conformer_dict['rd_mol']
                # ssr = Chem.GetSymmSSSR(rdkit_mol)
                # if len(ssr) < 1: continue
                num = Chem.Lipinski.NumAromaticRings(rdkit_mol)
                # if num < 1:continue
                ...
                # data.y = torch.tensor([len(ssr)])
                data.y = torch.tensor([num])
        ...
        if self.type_view in ['smiles', 'fuse']:
            ...
            roberta_tensor = torch.load('./Zero_Aromic_Ring_tensor.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_mol', type=int, default=10000, help='number of unique smiles/molecules')
    ...
    if args.sum:
        ...
        # n_mol, n_conf = 1000000, 5
            ...
            root = '../datasets/GEOM/processed/Case_Zero_AromRing_nmol_%d_Roberta' % (n_mol)
