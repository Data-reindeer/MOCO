    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in range(len(smiles_list)):
                ...
                # ========= manually add mol fingerprint and roberta_tensor =========
                fingerprint = np.array(GetMorganFingerprintAsBitVect(rdkit_mol, 2, nBits=1024))
                data.fingerprint = torch.tensor(fingerprint, dtype = torch.float).unsqueeze(0)
                data.roberta_tensor = roberta_tensor[i].unsqueeze(0)
                # ===================================================================
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:
                    ...
                    fingerprint = np.array(GetMorganFingerprintAsBitVect(rdkit_mol, 2, nBits=1024))
                    data.fingerprint = torch.tensor(fingerprint, dtype = torch.float).unsqueeze(0)
                    data.roberta_tensor = roberta_tensor[j].unsqueeze(0)
                    data.y = torch.tensor([labels[i]])
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
                    j+=1
...
def create_all_datasets():
    ...
    for dataset_name in downstream_dir:
        ...
        dataset = MoleculeDataset(root, dataset=dataset_name, roberta_tensor=smiles_repr)
        print(dataset)
